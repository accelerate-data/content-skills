...
EXPECTED_NAME = "content-skills"
MANIFEST_PATHS = (
    Path(".claude-plugin/plugin.json"),
    Path(".codex-plugin/plugin.json"),
)
REQUIRED_FIELDS = ("name", "description", "version", "author", "repository", "license", "commands")
...
def is_allowed_commands_path(value: Any) -> bool:
    return value in ("./commands", "./commands/")
...
def validate(root: Path) -> list[str]:
    ...
    for relative_path, manifest in manifests.items():
        ...
        if not is_allowed_commands_path(manifest.get("commands")):
            errors.append(f"{relative_path}: commands must point to './commands'")

    commands_dir = root / "commands"
    if not commands_dir.is_dir():
        errors.append("commands/: directory is missing")
    elif not any(commands_dir.glob("*.md")):
        errors.append("commands/: no command markdown files found")
